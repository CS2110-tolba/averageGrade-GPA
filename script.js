function submit(){
    ...
    if((name.value.length == 0)){
        ...
        noti.innerHTML = "Input Error try to put gpa from 1 to 5 only";
        status = 1;
    }
    if((year1.value > 5)||(year1.value < 0)||(year1.value.length == 0)){
        year1.style.backgroundColor = "#ff5252";
        noti.innerHTML = "Input Error try to put gpa from 1 to 5 only";
        status = 1;
    }
    if((year2.value > 5)||(year2.value < 0)||(year2.value.length == 0)){
        year2.style.backgroundColor = "#ff5252";
        noti.innerHTML = "Input Error try to put gpa from 1 to 5 only";
        status = 1;
    }
    if((year3.value > 5)||(year3.value < 0)||(year3.value.length == 0)){
        year3.style.backgroundColor = "#ff5252";
        noti.innerHTML = "Input Error try to put gpa from 1 to 5 only";
        status = 1;
    }
    if((year4.value > 5)||(year4.value < 0)||(year4.value.length == 0)){
        year4.style.backgroundColor = "#ff5252";
        noti.innerHTML = "Input Error try to put gpa from 1 to 5 only";
        status = 1;
    }
    if((year5.value > 5)||(year5.value < 0)||(year5.value.length == 0)){
    ...
    if(status == 1){return 0;}
        ...
        GPA = (parseFloat(year1.value) + parseFloat(year2.value) + parseFloat(year3.value) + parseFloat(year4.value) + parseFloat(year5.value))/5;
